...
- Your goal is to implement the **single task** specified in `REPO-ROOT/.github/workspace/Copilot_Design.md`.
...
### If nothing: you were accidentally stopped. Continue your work. Read `Copilot_Impl.md` to find where you left off.

## Step 2. Apply Code Changes

- Read the `# SPECIFICATION` in `Copilot_Design.md`.
...
- If you spot any difference between what the design planned and the actual source code:
  - It means I edited it. I have my reason. DO NOT revert my changes.
  - Note the difference in `# UPDATES` as `## User Update Spotted`.
  - Adapt your work around my changes.
...
- Go over each changed file and ensure:
...
## Step 6. Note Documentation Impact

- If the changes introduce new public APIs, change module behavior, or affect design documented in `doc/`:
...
## Step 7. Mark Task Complete and Backup

- After the code compiles cleanly and all tests pass:
  - Ensure there is a `# !!!VERIFIED!!!` mark at the end of `Copilot_Impl.md`.
  - In `Copilot_Scrum.md`, locate the task that was marked with `[x]` and change it to `[v]` to indicate completion.
  - Execute `REPO-ROOT/.github/scripts/copilotBackup.ps1 -TaskOnly` to back up `Copilot_Design.md` and `Copilot_Impl.md`.
    - This preserves a snapshot of the current task's design and implementation.
    - After the script completes, the workspace is ready for the next Design-Impl cycle.
